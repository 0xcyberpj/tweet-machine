...
output="$username.txt"
yes=`curl "http://web.archive.org/web/timemap/?url=https%3A%2F%2Ftwitter.com%2F$username%2F&matchType=prefix&collapse=urlkey&output=json&fl=original%2Cmimetype%2Ctimestamp%2Cendtimestamp%2Cgroupcount%2Cuniqcount&filter=!statuscode%3A[45]..&limit=100000&_=1627821432372" 2>/dev/null|grep twitter |cut -d '"' -f2 |tee $directory/$output`
